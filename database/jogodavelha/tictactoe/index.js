const fs = require('fs')
const dbdir = 'By_SR RIGBY'

function defineSave(db, obj, session) {
     mine = db
     const dbdir = `./database/jogodavelha/tictactoe/db/${session}.json`
...
function move(x, y, sessionS) {
     ...
     if (moving.isWin) {
          return { status: false, message: 'O jogo foi ganho por ' + moving.winner }
     }
     if (moving._matrix[x][y] == '❌') return { status: false, message: 'Este lugar foi preenchido por ❌' }
     if (moving._matrix[x][y] == '⭕') return { status: false, message: 'Este lugar foi preenchido por ⭕' }
     if (moving.turn == 'X') {
          moving._matrix[x][y] = '❌'
